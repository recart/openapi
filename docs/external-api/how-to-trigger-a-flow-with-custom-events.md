...
> **Custom events do not collect consent.** A custom event only triggers messaging for a phone number that is _already_ an opted-in SMS subscriber. If you need to subscribe someone as part of the same integration, use an event source with the `optin` type instead — see [How to Setup a Subscription Flow](how-to-setup-a-subscription-flow-with-recart-api).
...
1.  **Prepare Event Source**: Specify the event source type as "custom", along with the `category` and `name` that identify your event.
    ...
    Note the two different `type` fields: the outer `data.type` is the JSON:API resource type and must always be the literal `events`, even when creating an event _source_. The inner `attributes.type` is what selects `custom` or `optin`.
...
* The integration flow must be active. Flow state is _not_ validated when the event is posted: if no active flow uses the event source as its trigger, the event is still accepted with `200` and simply results in no message. When events succeed but nothing is sent, check the flow first
